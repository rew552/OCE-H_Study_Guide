...
> **重要**: Horizon REST API のベースパスは `/rest/` であり、`/rest/api/v1/` ではない。Swagger UI は `https://<CS>/rest/swagger-ui.html` で確認できる。API ドキュメントは Omnissa Developer Portal（`developer.omnissa.com/horizon-apis`）でもバージョンごとに参照可能。
...
> **重要**: 本番環境で API スクリプトを使用する場合、Swagger UI の `/rest/swagger-ui.html` でその Connection Server バージョンで利用可能なエンドポイントを必ず確認する。
...
> **重要**: ログアウトせずトークンを放置するとセキュリティリスクとなる。自動化スクリプトでは必ず `finally` ブロック等でログアウトを実行すべきである。
...
> **重要**: `Start-HVPool` は Push Image のほか、Refresh（リフレッシュ）、Recompose（再構成）、Rebalance（リバランス）の各メンテナンス操作を `-Maintenance` パラメータで指定して実行できる。
