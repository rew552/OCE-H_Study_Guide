...
## 0. Omnissa Access の基礎 / Omnissa Access Fundamentals

### 0.1 Omnissa Access とは

Omnissa Access は、Omnissa（旧 VMware EUC 部門）が提供する **ID プロバイダー（IdP）** および **アクセス管理プラットフォーム** である。

**名称の変遷**

| 時期 | 製品名 |
|------|--------|
| 〜2018 | VMware Identity Manager（vIDM） |
| 2018〜2024 | Workspace ONE Access |
| 2024〜 | **Omnissa Access** |

**主な役割**

- **Identity Provider（IdP）**: SAML 2.0 / OAuth 2.0 / OpenID Connect に対応した認証基盤
- **Single Sign-On（SSO）**: SaaS、Web アプリ、仮想デスクトップ/アプリへの統合ログイン
- **Conditional Access（条件付きアクセス）**: ネットワーク範囲・デバイス種別・デバイスコンプライアンス・認証方式に基づくアクセス制御
- **Application Catalog**: ユーザーが利用可能なアプリケーション・デスクトップを一覧表示するセルフサービスポータル

**デプロイモデル**

| モデル | 説明 | 特徴 |
|-------|------|------|
| **Cloud（SaaS）** | Omnissa がホスティングするクラウドサービス | インフラ管理不要、自動アップデート、Access Connector でオンプレミス AD と連携 |
| **On-Premises** | Access Appliance（Linux VA）をオンプレミスにデプロイ | 完全な自社管理、FedRAMP 等の規制要件対応、アップグレードは手動 |

### 0.2 主要コンポーネント

| コンポーネント | 説明 |
|-------------|------|
| **Access Service**（Cloud）/ **Access Appliance**（On-Prem） | IdP エンジン本体。認証処理、ポリシー評価、SAML Assertion 発行を行う |
| **Access Connector** | オンプレミス環境にデプロイする軽量コネクタ。以下の3つのサービスで構成される |
| — Directory Sync Service | Active Directory / LDAP からユーザー・グループを同期 |
| — User Auth Service | AD パスワード認証・RADIUS 認証をクラウドに中継 |
| — Kerberos Auth Service | Windows 統合認証（Kerberos）を処理。内部ネットワーク SSO に使用 |
| **Catalog** | ユーザーに公開するアプリケーション・デスクトップのポータル。Web / SaaS / Horizon / Citrix / ThinApp に対応 |
| **Authentication Methods** | Password（Cloud / Local）、Certificate、RADIUS、Kerberos、SAML（外部 IdP）、Mobile SSO など |
| **Access Policies** | 条件付きアクセスのルールエンジン。Network Range × Device Type × Authentication Method の組み合わせでルールを定義 |

### 0.3 アーキテクチャ概要

Cloud デプロイモデルにおける基本的な通信フローを以下に示す。
...
```
           ...
           / Artifact    │      │ HTTPS (443)
                ...
                └────────┬──────────┘
                         │
            SAML Artifact│ / Blast / PCoIP
                         │
                ┌────────▼──────────┐
                │ Horizon Connection │
                │ Server (SP)        │◄── SAML Artifact Resolve ──► Access
                └────────────────────┘

  ┌─────────────────────┐          ┌──────────────────┐
  │  Access Connector    │─────────►│  Active Directory │
  │  (Directory Sync /   │  LDAP /  │                  │
  │   User Auth /        │  Kerberos│                  │
  │   Kerberos Auth)     │          │                  │
  └─────────────────────┘          └──────────────────┘
...
Access Connector は **Outbound（アウトバウンド）接続** のみで Access Service と通信する。ファイアウォールで Inbound ポートを開放する必要がないため、DMZ 構成をシンプルに保てる。

### 0.4 Horizon との関係

Omnissa Access は Horizon の **外部 IdP** として機能し、以下の連携を実現する。

| 連携機能 | 説明 |
|---------|------|
| **SAML 2.0 認証委任（Authentication Delegation）** | Horizon Connection Server が認証処理を Access に委任。ユーザーは Access で認証後、SAML Assertion により Horizon リソースにアクセスする |
| **True SSO** | SAML Assertion のみで Windows デスクトップセッションにログオン可能にする仕組み。AD パスワード入力が不要になる（Enrollment Server + Enterprise CA が必要） |
| **条件付きアクセス** | デバイスコンプライアンス、ネットワーク範囲、認証方式に基づき Horizon リソースへのアクセスを制御 |
| **Application Catalog** | Horizon のデスクトップ・アプリケーションを Access のカタログに統合し、SaaS アプリと同じポータルから起動可能にする |

### 0.5 Horizon 統合に関連する主要用語

| 用語 | 説明 |
|------|------|
| **SP（Service Provider）** | SAML における「サービス提供者」。Horizon 統合では **Horizon Connection Server** が SP にあたる |
| **IdP（Identity Provider）** | SAML における「ID 提供者」。Horizon 統合では **Omnissa Access** が IdP にあたる |
| **SAML Authenticator** | Horizon Console で設定する IdP 定義。Dynamic（Access 用）と Static（UAG/サードパーティ用）の2種類がある |
| **SAML Assertion** | IdP がユーザー認証の結果を SP に伝達する XML トークン。ユーザー名・認証方式・有効期限などを含む |
| **SAML Artifact** | SAML Assertion への参照（ポインタ）。クライアント経由で SP に渡され、SP が IdP に Artifact Resolve して Assertion を取得する |
| **Virtual App Collection** | Access Console で Horizon Pod の接続情報・同期設定を定義するリソースグループ。Universal Broker を使用しない環境で Horizon リソースをカタログに追加する方式 |
| **Pod** | Access が接続する Horizon 環境の単位。1つの Virtual App Collection に複数の Pod を登録可能 |
| **True SSO** | Enrollment Server と Enterprise CA を使用し、短期証明書で Windows セッションへの SSO を実現する仕組み |
...
> **重要**: Default Access Policy は全てのアプリケーションに適用される。Horizon リソース専用の認証強化が必要な場合は、個別のアプリケーション固有ポリシーを作成する。
...
> **重要**: Metadata URL の `<YOUR HORIZON SERVER NAME>` を Access の FQDN に置き換える。URL の残りのパス（`/SAAS/API/1.0/GET/metadata/idp.xml`）はそのまま使用する。1つの Connection Server に複数の SAML Authenticator を設定可能だが、各 Authenticator の entity-ID は異なる必要がある。
...
> **重要**: メタデータ有効期限を延長しないと、24時間後に SAML 認証が失敗する。これは本番環境でよく見落とされる設定ポイント。
...
> **重要**: True SSO 用の証明書テンプレートは **Smartcard Logon** テンプレートの複製が基本。有効期間を非常に短く設定（通常1時間）することで、短期証明書がセッション確立時にのみ使用されるセキュリティモデルとなる。
...
> **重要**: True SSO には Enrollment Server、Microsoft AD CS（Enterprise CA）、適切な証明書テンプレートが必須。Standalone CA では動作しない。`vdmutil` は Connection Server 上の `C:\Program Files\Omnissa\Horizon\Server\tools` ディレクトリに存在する。
...
> **重要**: Universal Broker を使用していない環境では Virtual Apps Collection が必須。本セクションでは試験ガイドに沿い Virtual Apps Collection 方式を説明する。
...
> **重要**: Client Access FQDN はデフォルトで Connection Server の FQDN が設定される。本番環境では LB の名前空間に変更する必要がある。外部アクセスの場合は UAG の FQDN を設定する。
...
> **重要**: 外部クライアントの場合、UAG が認証トラフィックとプロトコルセッションの両方をプロキシする。Connection Server は直接外部に公開されない。
...
- [ ] Omnissa Access の役割（IdP、SSO、条件付きアクセス、アプリケーションカタログ）を説明できる
- [ ] Omnissa Access の名称変遷（vIDM → Workspace ONE Access → Omnissa Access）を把握している
- [ ] Cloud デプロイと On-Premises デプロイの違いを説明できる
- [ ] Access Connector の3つのサービス（Directory Sync / User Auth / Kerberos Auth）の役割を説明できる
- [ ] Horizon 統合における SP（Connection Server）と IdP（Access）の関係を説明できる
- [ ] SAML Assertion と SAML Artifact の違いを説明できる
